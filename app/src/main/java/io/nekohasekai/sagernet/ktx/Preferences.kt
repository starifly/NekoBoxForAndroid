...
fun PreferenceDataStore.string(name: String, defaultValue: () -> String = { "" }) =
    PreferenceProxy(name, defaultValue, ::getString, ::putString)

fun PreferenceDataStore.boolean(name: String, defaultValue: () -> Boolean = { false }) =
    PreferenceProxy(name, defaultValue, ::getBoolean, ::putBoolean)

fun PreferenceDataStore.int(name: String, defaultValue: () -> Int = { 0 }) =
    PreferenceProxy(name, defaultValue, ::getInt, ::putInt)

fun PreferenceDataStore.stringSet(name: String, defaultValue: () -> Set<String> = { setOf() }) =
    PreferenceProxy(name, defaultValue, ::getStringSet, ::putStringSet)

fun PreferenceDataStore.stringToInt(name: String, defaultValue: () -> Int = { 0 }) =
    PreferenceProxy(name, defaultValue, { key, default ->
        getString(key, "$default")?.toIntOrNull() ?: default
    }, { key, value -> putString(key, "$value") })

fun PreferenceDataStore.stringToIntIfExists(name: String, defaultValue: () -> Int = { 0 }) =
    PreferenceProxy(name, defaultValue, { key, default ->
        getString(key, "$default")?.toIntOrNull() ?: default
